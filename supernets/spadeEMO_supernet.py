...
class SPADEEMOSupernet(BaseSPADEDistiller, BaseEvolution):
    @staticmethod
    def modify_commandline_options(parser, is_train):
        parser = super(SPADEEMOSupernet, SPADEEMOSupernet).modify_commandline_options(parser, is_train)
    ...
    def __init__(self, opt):
        ...
        super(SPADEEMOSupernet, self).__init__(opt)
    ...
    def optimize_parameters(self, steps):
        ...
        super(SPADEEMOSupernet, self).optimize_parameters(steps)
    ...
    def evaluate_model_for_ec(self, child):
        ...
        for i, data_i in enumerate(tqdm(self.eval_dataloader, desc='Eval       ', position=tqdm_position, leave=False)):
            ...
            if cnt>=self.eval_cnt:  ###### specified number of eval images
                break
    ...
    def profile(self, config=None, verbose=True):
        if config is not None:
            self.modules_on_one_gpu.config = config
        macs, params = self.profile_spade(self.input_semantics[:1])
        if verbose:
            print('MACs: %.3fG\tParams: %.3fM' % (macs / 1e9, params / 1e6), flush=True)
        return macs, params

    def profile_spade(self, input_semantics):
        netG = self.modules_on_one_gpu.netG_student
        if isinstance(netG, torch.nn.DataParallel):
            netG = netG.module
        if self.modules_on_one_gpu.config is not None:
            netG.config = self.modules_on_one_gpu.config
        with torch.no_grad():
