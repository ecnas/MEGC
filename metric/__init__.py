...
from metric.fid_score import _compute_statistics_of_ims, calculate_frechet_distance, calculate_frechet_distance_fast
from metric.inception import InceptionV3
from utils import util


def get_fid(fakes, model, npz, device, batch_size=1, fast=False, tqdm_position=None):
    ...
    # print("time cost for calculate_frechet_distance:", time.time()-s2_begin, "fid:", fid)

    # s3_begin = time.time()
    if fast:
        fid = calculate_frechet_distance_fast(torch.Tensor(m1), torch.Tensor(s1), torch.Tensor(m2), torch.Tensor(s2))
    else:
        fid = calculate_frechet_distance(m1, s1, m2, s2)
    # print("time cost for frechet_distance:", time.time()-s3_begin, "fid2:", fid2)
