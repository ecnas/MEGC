...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    # print("cpu time:", time.time()-cpu_begin)
    # import pdb; pdb.set_trace()
    # device = torch.device('cuda:0')
    # gpu_begin = time.time()
    # sigma1_g = torch.from_numpy(sigma1).to(device)
    # sigma2_g = torch.from_numpy(sigma2).to(device)
    # t_g = sigma1_g.mm(sigma2_g)
    # print("gpu time:", time.time()-gpu_begin)
    # import pdb; pdb.set_trace()
...
def calculate_fid_given_ims(ims_fake, ims_real, batch_size, cuda, dims, model=None, tqdm_position=None):
    ...
    print("time cost for ims_fake:", time.time-s1_begin)
    s2_begin = time.time()
    m2, s2 = _compute_statistics_of_ims(ims_real, model, batch_size,
                                        dims, cuda, tqdm_position=tqdm_position)
    print("time cost for ims_real:", time.time-s2_begin)
    ss_begin = time.time()
    fid_value = calculate_frechet_distance(m1, s1, m2, s2)
    print("time cost for calculate_frechet_distance:", time.time-ss_begin)
    # import pdb; pdb.set_trace
    return fid_value
...
# def main():
#  fid_score.calculate_fid_given_paths(["database/horse2zebra/testA/n02381460_1000.jpg", "database/horse2zebra/testA/n02381460_1000.jpg"], 1, True, 2048)
...
def calculate_frechet_distance_fast(mu_x, sigma_x, mu_y, sigma_y):
    '''
    ...
    fid = (mu_x - mu_y).dot(mu_x - mu_y) + torch.trace(sigma_x) + torch.trace(sigma_y) 
    return  fid.item()
    #### END CODE HERE ####
